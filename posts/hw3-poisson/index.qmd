...
A key sanity check: fitting the identical model using `statsmodels.GLM` should produce coefficient estimates that match ours exactly (the log-likelihoods are the same problem) and standard errors that are very close. The prof's recipe says to use the Hessian for SEs — our numerical Hessian at convergence approximates the same Fisher information matrix that GLM uses internally, so the numbers should agree closely, with any small differences due to the numerical approximation of the Hessian.
...
(
    ...
    .tab_source_note("Custom MLE SEs computed from the negative inverse Hessian; GLM SEs are numerically identical.")
)
```
